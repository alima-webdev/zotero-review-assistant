...
export async function registerEventListener(element: HTMLElement | Document | undefined, eventType: string, fn: CustomEventFunction, options?: any) {
...
export async function deregisterEventListener(element: HTMLElement | Document | undefined, eventType: string, fn: CustomEventFunction, options?: any) {
    ...
    const eventId = events.findIndex(event => {
        if (event.element == element && event.eventType == eventType && event.fnId == fnId && event.options == options) return true
    })
...
export function deregisterAllEventListeners() {
    for(const event of events) {
        deregisterEventListener(event.element, event.eventType, event.fn, event.options)
    }
}
...
function getUniqueFunctionId(func: any) {
    ...
    return crypto.subtle.digest('SHA-256', new TextEncoder().encode(funcString))
        .then(hashBuffer => {
