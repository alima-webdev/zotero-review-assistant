...
`;
  ...
  var config = {
    ...
    releasePage: "https://github.com/alima-webdev/zotero-article-status/releases",
    updateJSON: "https://raw.githubusercontent.com/alima-webdev/zotero-article-status/main/update.json"
  };
...
`);
  ...
  function reportRegisterGlobalFunctions() {
    ztoolkit.getGlobal("document").generateReport = async () => {
      ...
      for (const status of allStatuses.filter(
        ...
        const statusLabel = status.label != "" ? status.label : getString("status-default-label");
  ...
  var ReviewModule = class {
    ...
    static registerExtraColumnWithBindings() {
      ...
      const contextMenu = statusContextMenu.concat(reasonContextMenu).concat(reportContextMenu);
