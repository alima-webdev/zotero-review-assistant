...
`;
  ...
  var Keystroke = class _Keystroke {
    ...
    static fromString(keystrokeString) {
      ...
      keystroke.modifiers.alt = keystrokeString.includes(keyString.alt) ? true : false;
      keystroke.modifiers.ctrl = keystrokeString.includes(keyString.ctrl) ? true : false;
      keystroke.modifiers.meta = keystrokeString.includes(keyString.meta) ? true : false;
      keystroke.modifiers.shift = keystrokeString.includes(keyString.shift) ? true : false;
  ...
  function removeAllStatuses(item) {
    item.getTags().map((tag) => {
      if (tag.tag.includes(statusTagPrefix))
  ...
  function initStatusColumn() {
    ...
    const getStatusColumnHook = (field, unformatted, includeBaseMapped, item) => {
  ...
  function statusKeyboardEvents(ev) {
    for (const status of allStatuses) {
      if (ev.altKey == status.keystroke.modifiers.alt && ev.ctrlKey == status.keystroke.modifiers.ctrl && ev.metaKey == status.keystroke.modifiers.meta && ev.shiftKey == status.keystroke.modifiers.shift && ev.key == status.keystroke.key) {
  ...
  function initAutoComplete(input, container) {
    autocomplete({
      ...
      fetch: function(text, update) {
        ...
        const suggestions = document.allReasons.filter(
...
`);
  ...
  function initReasonColumn() {
    ...
    const getReasonColumnHook = (field, unformatted, includeBaseMapped, item) => {
  ...
  __decorateClass([
    module
  ], ReviewModule, "registerDOMElements", 1);
  ...
  async function loadStatusTable(_window) {
    return new Promise((resolve, reject) => {
      ...
      statusTable = new ztoolkit.VirtualizedTable(_window).setContainerId(`${config.addonRef}-table-container`).setProp({
      ...
      }).setProp("getRowCount", () => allStatuses.length || 0).setProp("getRowData", (index) => allStatuses[index] || {}).setProp("getRowString", (index) => allStatuses[index].title || "").setProp("onSelectionChange", (selection) => {
  ...
  function bindPrefEvents(_window) {
    _window.document.querySelector("#btn-add-status")?.addEventListener("click", (ev) => {
      addStatus();
    });
    _window.document.querySelector("#btn-remove-status")?.addEventListener("click", (ev) => {
      removeStatus(selectedRow);
    });
    _window.document.querySelector(`#zotero-prefpane-${config.addonRef}-enable`)?.addEventListener("command", (e) => {
      ztoolkit.log(e);
      _.window.alert(
        `Successfully changed to ${e.target.checked}!`
      );
    });
    _window.document.querySelector(`#zotero-prefpane-${config.addonRef}-input`)?.addEventListener("change", (e) => {
      ztoolkit.log(e);
